 ...
 *
 *   CONVENTIONS:
 *     - Matrix structure is defined as row-major (memory layout) but
 *parameters naming AND all math operations performed by the library consider
 *the structure as it was column-major It is like transposed versions of the
...
RMAPI float
Remap (float value, float inputStart, float inputEnd, float outputStart,
       float outputEnd)
{
...
// triangle (a, b, c) NOTE: Assumes P is on the plane of the triangle
RMAPI Vector3
...
MatrixDeterminant (Matrix mat)
{
  ...
  result
      = a30 * a21 * a12 * a03 - a20 * a31 * a12 * a03 - a30 * a11 * a22 * a03
        ...
        - a00 * a21 * a12 * a33 - a10 * a01 * a22 * a33
        + a00 * a11 * a22 * a33;
...
RMAPI Matrix
MatrixFrustum (double left, double right, double bottom, double top,
...
RMAPI Matrix
MatrixPerspective (double fovY, double aspect, double nearPlane,
                   double farPlane)
{
  Matrix result = { 0 };
...
RMAPI Matrix
MatrixOrtho (double left, double right, double bottom, double top,
