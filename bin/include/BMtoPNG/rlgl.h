 ...
 *
 *       When an internal state change is required all the stored vertex data
 *is renderer in batch, additionally, rlDrawRenderBatchActive() could be called
 *to force flushing of the batch.
 *
 *       Some resources are also loaded for convenience, here the complete
 *list:
 ...
 *           Generates the implementation of the library into the included
 *file. If not defined, the library is in header only mode and can be included
 ...
 *default to shader location: 5 #define RL_DEFAULT_SHADER_UNIFORM_NAME_MVP
 *"mvp"               // model-view-projection matrix #define
...
#include <stdbool.h>
               ...
               // Matrix, 4x4 components, column major, OpenGL style, right
               // handed
...
// just forces a batch draw call if any of those state-change happens (this is
// done in core module)
...
{ // Prevents name mangling of functions
  ...
  RLAPI void
  rlColor4ub (unsigned char r, unsigned char g, unsigned char b,
  ...
  RLAPI void
  rlEnableVertexBuffer (unsigned int id);  // Enable vertex buffer (VBO)
  ...
  RLAPI void
  rlEnableTextureCubemap (unsigned int id);  // Enable texture cubemap
  ...
  // Framebuffer state
  RLAPI void
  rlEnableFramebuffer (unsigned int id); // Enable render texture (fbo)
  ...
  RLAPI bool
  rlIsStereoRenderEnabled (void); // Check if stereo render is enabled
  ...
  rlSetBlendFactors (int glSrcFactor, int glDstFactor,
                     int glEquation); // Set blending mode factor and equation
  ...
  RLAPI void
  rlglClose (void); // De-initialize rlgl (buffers, shaders, textures)
  ...
  RLAPI bool
  rlCheckRenderBatchLimit (int vCount); // Check internal buffer overflow for a
  ...
  RLAPI void
  rlUpdateVertexBuffer (unsigned int bufferId, const void *data, int dataSize,
  ...
  rlLoadTextureDepth (int width, int height,
                      bool useRenderBuffer); // Load depth texture/renderbuffer
                                             // (to be attached to fbo)
  ...
  RLAPI void
  rlUnloadTexture (unsigned int id); // Unload texture from GPU memory
  ...
  RLAPI void
  rlUnloadFramebuffer (unsigned int id); // Delete framebuffer from GPU
  ...
  RLAPI void rlComputeShaderDispatch (
      ...
      unsigned int groupZ); // Dispatch compute shader (equivalent to *draw*
  ...
  RLAPI void
  rlUpdateShaderBuffer (unsigned int id, const void *data,
  ...
  rlGetMatrixViewOffsetStereo (int eye); // Get internal view offset matrix for
                                         // stereo render (selected eye)
  RLAPI void
  rlSetMatrixProjection (Matrix proj); // Set a custom projection matrix
                                       // (replaces internal projection matrix)
  RLAPI void
  rlSetMatrixModelview (Matrix view); // Set a custom modelview matrix
                                      // (replaces internal modelview matrix)
...
// Pi provided headers (despite being defined in official Khronos GLES2
// headers)
...
void
rlFrustum (double left, double right, double bottom, double top, double znear,
           double zfar)
{
...
void
rlOrtho (double left, double right, double bottom, double top, double znear,
         double zfar)
{
...
// Multiply the current matrix by an orthographic matrix generated by
// parameters
void
rlOrtho (double left, double right, double bottom, double top, double znear,
         double zfar)
...
rlVertex3f (float x, float y, float z)
{
  ...
  // Add vertices
  RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
      .vertices[3 * RLGL.State.vertexCounter]
      = tx;
  RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
      .vertices[3 * RLGL.State.vertexCounter + 1]
      = ty;
  RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
      .vertices[3 * RLGL.State.vertexCounter + 2]
      = tz;

  // Add current texcoord
  RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
      .texcoords[2 * RLGL.State.vertexCounter]
      = RLGL.State.texcoordx;
  RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
      .texcoords[2 * RLGL.State.vertexCounter + 1]
  ...
  // Add current color
  RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
      .colors[4 * RLGL.State.vertexCounter]
      = RLGL.State.colorr;
  RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
      .colors[4 * RLGL.State.vertexCounter + 1]
      = RLGL.State.colorg;
  RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
      .colors[4 * RLGL.State.vertexCounter + 2]
      = RLGL.State.colorb;
  RLGL.currentBatch->vertexBuffer[RLGL.currentBatch->currentBuffer]
      .colors[4 * RLGL.State.vertexCounter + 3]
      = RLGL.State.colora;
          ...
          != id)
        {
          if (RLGL.currentBatch->draws[RLGL.currentBatch->drawCounter - 1]
              ...
              > 0)
            {
              ...
              // will keep aligned with index processing It implies adding some
              // extra alignment vertex at the end of the draw, those vertex
              // are not processed but they are considered as an additional
              // offset for the next set of vertex to be drawn
              if (RLGL.currentBatch->draws[RLGL.currentBatch->drawCounter - 1]
                      .mode
                  == RL_LINES)
                RLGL.currentBatch->draws[RLGL.currentBatch->drawCounter - 1]
...
void
rlBlitFramebuffer (int srcX, int srcY, int srcWidth, int srcHeight, int dstX,
                   int dstY, int dstWidth, int dstHeight, int bufferMask)
{
...
void
rlClearColor (unsigned char r, unsigned char g, unsigned char b,
              unsigned char a)
{
...
rlClearScreenBuffers (void)
{
  glClear (GL_COLOR_BUFFER_BIT
           | GL_DEPTH_BUFFER_BIT); // Clear used buffers: Color and Depth
                                   // (Depth is used for 3D)
...
void
rlSetBlendFactorsSeparate (int glSrcRGB, int glDstRGB, int glSrcAlpha,
...
rlDebugMessageCallback (GLenum source, GLenum type, GLuint id, GLenum severity,
...
rlglInit (int width, int height)
{
  ...
  glClear (GL_COLOR_BUFFER_BIT
           | GL_DEPTH_BUFFER_BIT); // Clear color and depth buffers (depth
                                   // buffer required for 3D)
}
  ...
  for (int i = 0; i < numExt; i++)
    {
      ...
      if ((strcmp (extList[i], (const char *)"GL_EXT_texture_compression_s3tc")
           == 0)
          || (strcmp (extList[i],
                      (const char *)"GL_WEBGL_compressed_texture_s3tc")
              == 0)
          || (strcmp (extList[i],
      ...
      if ((strcmp (extList[i],
           ...
           == 0)
          || (strcmp (extList[i],
                      (const char *)"GL_WEBGL_compressed_texture_etc1")
              == 0))
        RLGL.ExtSupported.texCompETC1 = true;
      ...
      if (strcmp (extList[i],
                  (const char *)"GL_KHR_texture_compression_astc_hdr")
          == 0)
        RLGL.ExtSupported.texCompASTC = true;
  ...
  if (RLGL.State.vertexCounter > 0)
    {
      ...
      // first glBufferData() with NULL pointer before glMapBuffer(). If you do
      ...
      if (RLGL.State.vertexCounter > 0)
        {
          ...
          glUniformMatrix4fv (
              RLGL.State.currentShaderLocs[RL_SHADER_LOC_MATRIX_MVP], 1, false,
              matMVPfloat);
          ...
          // texture0 changes, additional textures are enabled for all the draw
          // calls
          ...
          for (int i = 0, vertexOffset = 0; i < batch->drawCounter; i++)
            {
              ...
              if ((batch->draws[i].mode == RL_LINES)
              ...
              else
                {
                  ...
                  // elementCount*6 NOTE: The final parameter tells the GPU the
                  // offset in bytes from the start of the index buffer to the
                  // location of the first index to process
                  glDrawElements (
...
unsigned int
rlLoadTexture (const void *data, int width, int height, int format,
               int mipmapCount)
{
  unsigned int id = 0;
      ...
      if (glInternalFormat != 0)
        {
          if (format < RL_PIXELFORMAT_COMPRESSED_DXT1_RGB)
            ...
            glCompressedTexImage2D (GL_TEXTURE_2D, i, glInternalFormat,
                                    mipWidth, mipHeight, 0, mipSize, dataPtr);
#endif
...
rlReadTexturePixels (unsigned int id, int width, int height, int format)
{
  ...
  // get it from texture id. Two possible Options: 1 - Bind texture to color
  // fbo attachment and glReadPixels() 2 - Create an fbo, activate it, render
  // quad with texture, glReadPixels() We are using Option 1, just need to care
  // for texture format on retrieval NOTE: This behaviour could be conditioned
  // by graphic driver...
...
{
  unsigned char *screenData = (unsigned char *)RL_CALLOC (
  ...
  unsigned char *imgData = (unsigned char *)RL_MALLOC (
      width * height * 4 * sizeof (unsigned char));
...
void
rlUpdateVertexBuffer (unsigned int id, const void *data, int dataSize,
                      int offset)
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
  glBindBuffer (GL_ARRAY_BUFFER, id);
...
// NOTE: dataSize and offset must be provided in bytes
void
rlUpdateVertexBufferElements (unsigned int id, const void *data, int dataSize,
                              int offset)
{
#if defined(GRAPHICS_API_OPENGL_33) || defined(GRAPHICS_API_OPENGL_ES2)
  glBindBuffer (GL_ELEMENT_ARRAY_BUFFER, id);
...
void
rlUpdateShaderBuffer (unsigned int id, const void *data, unsigned int dataSize,
                      unsigned int offset)
{
#if defined(GRAPHICS_API_OPENGL_43)
...
// Bind image texture
void
rlBindImageTexture (unsigned int id, unsigned int index, int format,
                    bool readonly)
{
#if defined(GRAPHICS_API_OPENGL_43)
